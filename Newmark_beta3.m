function [Tv_all, T, X_all] = Newmark_beta3(config, pre, radial, coupled, varargin)
% Newmark_beta3  engine of plastic structural response for MH bar
...
%   radial              logical, whether to consider radial forces 
...
num_mesh = length(EA_all);
num_mat = length(E);
k_key = floor((1 - pre.ls/pre.l)*num_mesh);
R = (Coord(1:end-1,2) + Coord(2:end,2))/2;
U = zeros(size(M,1),length_t);      dU = U;     ddU = U;    U_real = U;     dU_real = U;    ddU_real = U;

EA_all_all = zeros(num_mesh, length_t);     Ga_all_all = EA_all_all;    Epsilonx = EA_all_all;      Phi = EA_all_all;
...
D_2G = zeros(1, num_mat);
for j = 1:num_mat
    Epsilon_turning(:,j) = Y(j)/2/G(j);
    D_2G(j) = 2*G(j)*D(j)/E(j);
end
...
for i = 1:length_t-1 
    ...
    if coupled
        ...
        [dX1, P_now1, ~, Fr1] = deformable(X, U_real(:,i), dU_real(:,i), config, pre);
        dt = dt_estimator(dt_default, dX1);
        T(i+1) = t + dt;
        [dX2, P_now2, ~, Fr2] = deformable(X + dt*dX1/2, U_real(:,i), dU_real(:,i), config, pre);
        [dX3, P_now3, ~, Fr3] = deformable(X + dt*dX2/2, U_real(:,i), dU_real(:,i), config, pre);
        [dX4, P_now4, ~, Fr4] = deformable(X + dt*dX3, U_real(:,i), dU_real(:,i), config, pre);
        ...
        if radial
            ...
            P(:,i+1) = [P_now; zeros(num_mesh-1,1)];
        end
    else
        dt = T(i+1) - T(i);
    end
    ...
    Epsilonx_try = Epsilonx(:,i);           Phi_try = Phi(:,i);
    tim = 1;
    ...
    while true
        ...
        delta_Epsilonx_try = vibration_dX(Coord, delta_U_try(1:num_mesh+1));
        delta_Phi_try = ([0; delta_U_try(num_mesh + 2:end)] + [delta_U_try(num_mesh + 2:end); 0])/2./R;
        delta_Epsilon_try = delta_Epsilonx_try - delta_Phi_try;
                ...
                for j = 1:num_mat     
                    if A_all_all(k,j) == 0
                        ...
                        [Epsilon_turning(k,j), Epsilon_turning2(k,j), delta_Sigma_try(k,j), delta_Epsilon_p_try(k,j)] = constitution_sigma(...
                         Epsilon_turning(k,j), Epsilon_turning2(k,j), Epsilonx_try(k)-Phi_try(k), delta_Epsilon_try(k), Sigma(k,i,j), ...
                         Y(j), 2*G(j), D_2G(j), consti(j));
                        
                        poisson = (delta_Epsilon_p_try(k,j)*0.49 + (delta_Epsilon_try(k) - delta_Epsilon_p_try(k,j))*Poisson(j))/delta_Epsilon_try(k);
                    end
                    G_try = delta_Sigma_try(k,j)/delta_Epsilon_try(k)/2;
                    EA_all_try(k) = EA_all_try(k) + G_try*A_all_all(k,j)*2*(1+poisson);
                    GA_all_try(k) = GA_all_try(k) + G_try*A_all_all(k,j);
                    GI_all_try(k) = GI_all_try(k) + G_try*I_all_all(k,j);
                end
            end
        end
        ...
        Epsilonx_try = Epsilonx_try + delta_Epsilonx_try;
        Phi_try = Phi_try + delta_Phi_try;
...
%         norm(delta_P_e_try - delta_P_e)/norm(delta_P_e)
    ...
    Epsilonx(:,i+1) = Epsilonx_try;     Phi(:,i+1) = Phi_try;
    for j = 1:num_mat
    ...
    delta_U_real  = delta_U;
    delta_Epsilon_p2 = vibration_dEp_fix(delta_Epsilon_p(1:k_key+1,1));
    delta_U_real(num_mesh + 2: num_mesh + 2 + k_key) = delta_U_real(num_mesh + 2: num_mesh + 2 + k_key) + 0.658*delta_Epsilon_p2.*R(1:k_key+1);
    [U_real(:,i+1), dU_real(:,i+1), ddU_real(:,i+1)] = Newmark_beta_iter(U_real(:,i), dU_real(:,i), ddU_real(:,i), delta_U_real, gamma, beta, dt);  
...
T = T(1:i);                                Tv_all.TUX = (U_real(:,1:i))';
Tv_all.TVX = (dU_real(:,1:i))';            Tv_all.TAX = (ddU_real(:,1:i))';    
Tv_all.Epsilon = Epsilonx(:,1:i);          Tv_all.Epsilon_p = Epsilon_p(:,1:i, :);
