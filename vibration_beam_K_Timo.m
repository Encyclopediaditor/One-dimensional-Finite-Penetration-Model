function K_all = vibration_beam_K_Timo(Coord, GA_all, EI_all, N, cut_edge, tip_fix, varargin)
% vibration_beam_K_Timo  generate K matrix for beam models, Timoshenko type
...
for i = 1 : num_element
   ...
   n = vibration_beam_Key_Timo(Phi, L, 'n');
   
   K =  EI_all(i)/L^3/(1+Phi)*[12,   6*L,   -12,   6*L;
                               6*L, (Phi+4)*L^2, -6*L, (2-Phi)*L^2;
                               -12,  -6*L,   12,  -6*L;
                               6*L, (2-Phi)*L^2, -6*L, (Phi+4)*L^2];
   K = K + N(i)/L/((1+Phi)^2)*[n(1) n(2) -n(1) n(2);
                               n(2) n(3) -n(2) n(4);
                               -n(1) -n(2) n(1) -n(2);
                               n(2) n(4) -n(2) n(3)];
